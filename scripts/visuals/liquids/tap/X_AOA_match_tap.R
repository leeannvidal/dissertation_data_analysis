# scripts/visuals/liquids/tap/AOA_match_tap.R

#Calculate chi-square to include in plot AOA
## Create a Contingency Table
tap_AOA_match <- table(tap$PHONETIC_PHONOLOGICAL_AGREEMENT, tap$AOA)
# 
# # Run χ² function -
chi_label_AOA_tap_match <- create_chi_label(chisq.test(tap_AOA_match))
# 
chi_label_AOA_tap_match
# 
unique(tap$AOA)
...
ggplot(tap_mismatch, aes(x = PHONETIC_PHONOLOGICAL_AGREEMENT, y = AOA)) +
  geom_jitter(width = 0.2) +
  labs(x = "Phonetic/Phonological Agreement", y = "Age of Arrival") +
  theme_minimal()

AOA_plot_tap_match <- tap %>%
  ...
  # geom_text(
  #   aes(label = ifelse(AOA == "male" & PHONETIC_PHONOLOGICAL_AGREEMENT == "match", chi_label_AOA_tap_match, "")),
...
# ggsave("output/plots/liquids/tap/AOA_plot_tap_match.pdf", AOA_plot_tap_match,device = cairo_pdf, width = 12, height = 7) 
