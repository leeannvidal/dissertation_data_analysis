# scripts/visuals/liquids/tap/Percent_Spanish_Interlocutor_x_Mismatch_Rate_DR_tap.R

### Run linear regression to label plot

# Fit the linear model
PERCENT_INTL_SPAN_ONLY_model_tap_dr <- lm(RATE_MISMATCH_TAP ~ PERCENT_INTL_SPAN_ONLY, data = dr_rates_mismatch_all)

# # Run R² function to create label-
PERCENT_INTL_SPAN_ONLY_r2_pvalue_label_tap_dr <- create_regression_label(summary(PERCENT_INTL_SPAN_ONLY_model_tap_dr))
# PERCENT_INTL_SPAN_ONLY_r2_pvalue_label_tap_dr
# Plot the data - tap

# Create the plot with annotations for R^2 and p-value
PERCENT_INTL_SPAN_ONLY_MISMATCH_RATE_DR_tap <- dr_rates_mismatch_all %>%
  ...
  labs(x = "% of Spanish only with Interlocutors", y = "Mismatch Rate (%)\n",
       title = "Rate of Phonetic-Phonological Mismatch for /ɾ/ by Percent of Spanish only with Interlocutors for Dominican Speakers",
       caption = "Data Source: Spanish in Boston Corpus \n Vidal-Covas Dissertation"
  ) +
  basic_custom_theme() +
  scale_y_continuous(limits = c(0, 100))  # Set the y-axis limits from 0 to 100

PERCENT_INTL_SPAN_ONLY_MISMATCH_RATE_DR_tap
