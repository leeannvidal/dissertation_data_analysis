# scripts/visuals/liquids/tap/previous_sound_category_match_tap.R

#Calculate chi-square to include in plot PREV_SOUND_CATEGORY
## Create a Contingency Table
tap_PREV_SOUND_CATEGORY_match <- table(tap$PHONETIC_PHONOLOGICAL_AGREEMENT, tap$PREV_SOUND_CATEGORY)

# # Run χ² function -
chi_label_PREV_SOUND_CATEGORY_tap_match <- create_chi_label(chisq.test(tap_PREV_SOUND_CATEGORY_match))

PREV_SOUND_CATEGORY_plot_tap_match <- tap %>%
  ...
  geom_text(
    aes(label = ifelse(PREV_SOUND_CATEGORY == "vowel" & PHONETIC_PHONOLOGICAL_AGREEMENT == "match", chi_label_PREV_SOUND_CATEGORY_tap_match, "")),
  ...
  labs(
    ...
    y= "Proportion\n", 
    fill="", 
    title = "What is the proportional Phonetic/Phonological Agreement distribution\n of taps for Previous Sound Category?",
    caption = "Data Source: Spanish in Boston Corpus \n Vidal Covas' Dissertation Speakers"
      ) +
  scale_fill_economist() +
  basic_custom_theme()

PREV_SOUND_CATEGORY_plot_tap_match

# ggsave("output/plots/liquids/tap/PREV_SOUND_CATEGORY_plot_tap_match.pdf", PREV_SOUND_CATEGORY_plot_tap_match,device = cairo_pdf, width = 12, height = 7) 
