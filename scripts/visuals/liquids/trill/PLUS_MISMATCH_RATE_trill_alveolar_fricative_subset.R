...
# Plot the data - TRILL
RATE_MISMATCH_TRILL_SUBSET
# Create the plot with annotations for R^2 and p-value
PLUS_MISMATCH_RATE_TRILL_SUBSET <- rates_all_variables %>%
  # ggplot(aes(x = PLUS, y = RATE_MISMATCH_TRILL, color = COUNTRY_OF_ORIGIN)) +
  ggplot(aes(x = PLUS, y = RATE_MISMATCH_TRILL_SUBSET)) +
...
# +
  ...
  #          hjust = 1, vjust = 0, size = 5, color = "black",  # Right aligned, bottom aligned
...
PLUS_MISMATCH_RATE_TRILL_SUBSET

ggsave("output/plots/liquids/trill/PLUS_MISMATCH_RATE_TRILL_SUBSET.pdf", PLUS_MISMATCH_RATE_TRILL_SUBSET,device = cairo_pdf, width = 8, height = 8)
