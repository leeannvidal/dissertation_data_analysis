# scripts/visuals/liquids/trill/Percent_English_Interlocutor_x_Mismatch_Rate_DR_trill.R

### Run linear regression to label plot

# Fit the linear model
PERCENT_INTL_ENG_ONLY_model_trill_dr <- lm(RATE_MISMATCH_TRILL ~ PERCENT_INTL_ENG_ONLY, data = dr_rates_mismatch_all)

# # Run R² function to create label-
PERCENT_INTL_ENG_ONLY_r2_pvalue_label_trill_dr <- create_regression_label(summary(PERCENT_INTL_ENG_ONLY_model_trill_dr))
# PERCENT_INTL_ENG_ONLY_r2_pvalue_label_trill_dr
# Plot the data - TRILL

# Create the plot with annotations for R^2 and p-value
PERCENT_INTL_ENG_ONLY_MISMATCH_RATE_DR_trill <- dr_rates_mismatch_all %>%
  ...
  labs(x = "% of English only with Interlocutors", y = "Mismatch Rate (%)\n",
       title = "Rate of Phonetic-Phonological Mismatch for /r/ by Percent of English only with Interlocutors for Dominican Speakers",
       caption = "Data Source: Spanish in Boston Corpus \n Vidal-Covas Dissertation"
  ) +
  basic_custom_theme() +
  scale_y_continuous(limits = c(0, 100))  # Set the y-axis limits from 0 to 100

PERCENT_INTL_ENG_ONLY_MISMATCH_RATE_DR_trill
