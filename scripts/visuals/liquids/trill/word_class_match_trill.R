# scripts/visuals/liquids/trill/word_class_match_trill.R

##FIX THIS
#Calculate chi-square to include in plot WORD_CLASS
## Create a Contingency Table
trill_WORD_CLASS_match <- table(trill$PHONETIC_PHONOLOGICAL_AGREEMENT, trill$WORD_CLASS)
# 
# Remove the NA column (column with all zeros)
trill_WORD_CLASS_match <- trill_WORD_CLASS_match[, colSums(trill_WORD_CLASS_match) > 0]

# # Run χ² function -
chi_label_WORD_CLASS_trill_match <- create_chi_label(chisq.test(trill_WORD_CLASS_match))
# 
# 
WORD_CLASS_plot_Trill_match <- trill %>%
  ...
  geom_text(
    aes(label = ifelse(WORD_CLASS == "adverb" & PHONETIC_PHONOLOGICAL_AGREEMENT == "match", chi_label_WORD_CLASS_trill_match, "")),
...
WORD_CLASS_plot_Trill_match

# ggsave("output/plots/liquids/trill/WORD_CLASS_plot_Trill_match.pdf", WORD_CLASS_plot_Trill_match,device = cairo_pdf, width = 12, height = 7) 