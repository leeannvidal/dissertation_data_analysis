# scripts/visuals/liquids/lateral/city_of_origin_match_lateral.R

#Calculate chi-square to include in plot CITY_OF_ORIGIN
## Create a Contingency Table
lateral_CITY_OF_ORIGIN_match <- table(lateral$PHONETIC_PHONOLOGICAL_AGREEMENT, lateral$CITY_OF_ORIGIN)
# 
# # Run χ² function -
chi_label_CITY_OF_ORIGIN_lateral_match <- create_chi_label(chisq.test(lateral_CITY_OF_ORIGIN_match))
# 
chi_label_CITY_OF_ORIGIN_lateral_match
# 
unique(lateral$CITY_OF_ORIGIN)

CITY_OF_ORIGIN_plot_lateral_match <- lateral %>%
  ...
  # geom_text(
  #   aes(label = ifelse(CITY_OF_ORIGIN == "male" & PHONETIC_PHONOLOGICAL_AGREEMENT == "match", chi_label_CITY_OF_ORIGIN_lateral_match, "")),
...
CITY_OF_ORIGIN_plot_lateral_match

# ggsave("output/plots/liquids/lateral/CITY_OF_ORIGIN_plot_lateral_match.pdf", CITY_OF_ORIGIN_plot_lateral_match,device = cairo_pdf, width = 12, height = 7) 
