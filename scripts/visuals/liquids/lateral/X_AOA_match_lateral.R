# scripts/visuals/liquids/lateral/AOA_match_lateral.R

#Calculate chi-square to include in plot AOA
## Create a Contingency Table
lateral_AOA_match <- table(lateral$PHONETIC_PHONOLOGICAL_AGREEMENT, lateral$AOA)
# 
# # Run χ² function -
chi_label_AOA_lateral_match <- create_chi_label(chisq.test(lateral_AOA_match))
# 
chi_label_AOA_lateral_match
# 
unique(lateral$AOA)
...
ggplot(lateral_mismatch, aes(x = PHONETIC_PHONOLOGICAL_AGREEMENT, y = AOA)) +
  geom_jitter(width = 0.2) +
  labs(x = "Phonetic/Phonological Agreement", y = "Age of Arrival") +
  theme_minimal()

AOA_plot_lateral_match <- lateral %>%
  ...
  # geom_text(
  #   aes(label = ifelse(AOA == "male" & PHONETIC_PHONOLOGICAL_AGREEMENT == "match", chi_label_AOA_lateral_match, "")),
...
AOA_plot_lateral_match

# ggsave("output/plots/liquids/lateral/AOA_plot_lateral_match.pdf", AOA_plot_lateral_match,device = cairo_pdf, width = 12, height = 7) 
