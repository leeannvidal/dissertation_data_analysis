# scripts/visuals/liquids/lateral/following_sound_category_match_lateral.R
...
beta_label_PREV_SOUND_CAT_pause_mismatch_lateral <- get_estimate_and_pvalues(predictor = "FOLL_SOUND_CATEGORYpause", model_path = "data/regressions/best_model_lateral.rds")
beta_label_PREV_SOUND_CAT_vowel_mismatch_lateral <- get_estimate_and_pvalues(predictor = "FOLL_SOUND_CATEGORYvowel", model_path = "data/regressions/best_model_lateral.rds")


FOLL_SOUND_CATEGORY_plot_lateral_match <- lateral %>%
  ...
  # geom_text(
  #   aes(label = ifelse(FOLL_SOUND_CATEGORY == "vowel" & PHONETIC_PHONOLOGICAL_AGREEMENT == "match", beta_label_PREV_SOUND_CAT_vowel_mismatch_lateral, "")),
  #   position = position_fill(vjust = 0.55), # Adjust vertical position to center
  #   color = "white", size = 8, family = "charis", stat = "count") +
  # geom_text(
  #   aes(label = ifelse(FOLL_SOUND_CATEGORY == "pause" & PHONETIC_PHONOLOGICAL_AGREEMENT == "match", beta_label_PREV_SOUND_CAT_pause_mismatch_lateral, "")),
  #   position = position_fill(vjust = 0.55), # Adjust vertical position to center
  #   color = "white", size = 8, family = "charis", stat = "count") +
  # geom_hline(
...
FOLL_SOUND_CATEGORY_plot_lateral_match

ggsave("output/plots/liquids/lateral/FOLL_SOUND_CATEGORY_plot_lateral_match.pdf", FOLL_SOUND_CATEGORY_plot_lateral_match,device = cairo_pdf, width = 11, height = 7)
