# scripts/visuals/liquids/lateral/PLUS_x_Mismatch_Rate_PR_lateral.R


### Run linear regression to label plot

# Fit the linear model
PLUS_model_lateral_pr <- lm(RATE_MISMATCH_LATERAL ~ PLUS, data = pr_rates_mismatch_all)

# # Run R² function to create label-
PLUS_r2_pvalue_label_lateral_pr <- create_regression_label(summary(PLUS_model_lateral_pr))

# Plot the data - LATERAL

# Create the plot with annotations for R^2 and p-value
PLUS_MISMATCH_RATE_PR_lateral <- pr_rates_mismatch_all %>%
  ...
  labs(x = "PLUS", y = "Mismatch Rate (%)\n",
       title = "Rate of Phonetic-Phonological Mismatch for /l/ by PLUS for Puerto Rican Speakers",
       caption = "Data Source: Spanish in Boston Corpus \n Vidal-Covas Dissertation"
  ) +
  basic_custom_theme() +
  scale_y_continuous(limits = c(0, 100))  # Set the y-axis limits from 0 to 100

PLUS_MISMATCH_RATE_PR_lateral